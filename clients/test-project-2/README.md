...
Replace `test_project_2` with your actual project name throughout:

```bash
# Rename the directory
mv test_project_2 my_client

# Update all references (Linux/macOS):
find . -type f \( -name "*.py" -o -name "*.txt" -o -name "pytest.ini" \) \
  -exec sed -i 's/test_project_2/my_client/g' {} +

# Verify no stale references remain:
grep -r "test_project_2" .
```
...
| ------------------------------- | ----------------------------------------------- |
| `test_project_2/` directory       | Rename to your project name                     |
| `test_project_2/settings/base.py` | Update `WAGTAIL_SITE_NAME`                      |
| `test_project_2/home/apps.py`     | Update `name` and `label` to match              |
...
│
├── test_project_2/           # Your Django project package
│   ├── __init__.py
...
1. Set `DJANGO_SETTINGS_MODULE=test_project_2.settings.production`
2. Ensure all required environment variables are set (see `.env.example`)
3. Run `python manage.py collectstatic`
4. Deploy with gunicorn: `gunicorn test_project_2.wsgi:application`
