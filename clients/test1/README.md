...
Replace `test1` with your actual project name throughout:
...
find . -type f \( -name "*.py" -o -name "*.txt" -o -name "pytest.ini" \) \
  -exec sed -i 's/test1/my_client/g' {} +
...
| ------------------------------- | ----------------------------------------------- |
| `test1/` directory       | Rename to your project name                     |
| `test1/settings/base.py` | Update `WAGTAIL_SITE_NAME`                      |
| `test1/home/apps.py`     | Update `name` and `label` to match              |
...
│
├── test1/           # Your Django project package
│   ├── __init__.py
...
1. Set `DJANGO_SETTINGS_MODULE=test1.settings.production`
2. Ensure all required environment variables are set (see `.env.example`)
3. Run `python manage.py collectstatic`
4. Deploy with gunicorn: `gunicorn test1.wsgi:application`
