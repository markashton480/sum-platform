...
Replace `acme_kitchens` with your actual project name throughout:

```bash
# Rename the directory
mv acme_kitchens my_client

# Update all references (Linux/macOS):
find . -type f \( -name "*.py" -o -name "*.txt" -o -name "pytest.ini" \) \
  -exec sed -i 's/acme_kitchens/my_client/g' {} +

# Verify no stale references remain:
grep -r "acme_kitchens" .
```
...
| ------------------------------- | ----------------------------------------------- |
| `acme_kitchens/` directory       | Rename to your project name                     |
| `acme_kitchens/settings/base.py` | Update `WAGTAIL_SITE_NAME`                      |
| `acme_kitchens/home/apps.py`     | Update `name` and `label` to match              |
...
│
├── acme_kitchens/           # Your Django project package
│   ├── __init__.py
...
1. Set `DJANGO_SETTINGS_MODULE=acme_kitchens.settings.production`
2. Ensure all required environment variables are set (see `.env.example`)
3. Run `python manage.py collectstatic`
4. Deploy with gunicorn: `gunicorn acme_kitchens.wsgi:application`
