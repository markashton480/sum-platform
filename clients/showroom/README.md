...
Replace `showroom` with your actual project name throughout:

```bash
# Rename the directory
mv showroom my_client

# Update all references (Linux/macOS):
find . -type f \( -name "*.py" -o -name "*.txt" -o -name "pytest.ini" \) \
  -exec sed -i 's/showroom/my_client/g' {} +
...
| ------------------------------- | ----------------------------------------------- |
| `showroom/` directory       | Rename to your project name                     |
| `showroom/settings/base.py` | Update `WAGTAIL_SITE_NAME`                      |
| `showroom/home/apps.py`     | Update `name` and `label` to match              |
...
│
├── showroom/           # Your Django project package
│   ├── __init__.py
...
1. Set `DJANGO_SETTINGS_MODULE=showroom.settings.production`
2. Ensure all required environment variables are set (see `.env.example`)
3. Run `python manage.py collectstatic`
4. Deploy with gunicorn: `gunicorn showroom.wsgi:application`
