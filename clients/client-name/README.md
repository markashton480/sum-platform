...
Replace `client_name` with your actual project name throughout:

```bash
# Rename the directory
mv client_name my_client

# Update all references (Linux/macOS):
find . -type f \( -name "*.py" -o -name "*.txt" -o -name "pytest.ini" \) \
  -exec sed -i 's/client_name/my_client/g' {} +
...
| ------------------------------- | ----------------------------------------------- |
| `client_name/` directory       | Rename to your project name                     |
| `client_name/settings/base.py` | Update `WAGTAIL_SITE_NAME`                      |
| `client_name/home/apps.py`     | Update `name` and `label` to match              |
...
│
├── client_name/           # Your Django project package
│   ├── __init__.py
...
1. Set `DJANGO_SETTINGS_MODULE=client_name.settings.production`
2. Ensure all required environment variables are set (see `.env.example`)
3. Run `python manage.py collectstatic`
4. Deploy with gunicorn: `gunicorn client_name.wsgi:application`
