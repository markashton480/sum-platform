...
function toggleAccordion(id) {
    const wrapper = document.getElementById(id); // This is the grid wrapper
    ...
    if (isOpen) {
        ...
        icon.style.transform = 'rotate(0deg)'; // Optional: Rotate icon back
        icon.style.opacity = '0.3'; // Dim the icon
        button.setAttribute('aria-expanded', 'false');
    } else {
        // Open it
        wrapper.classList.add('open');
        icon.style.transform = 'rotate(45deg)'; // Optional: Rotate icon to X
        icon.style.opacity = '1'; // Light up the icon
        button.setAttribute('aria-expanded', 'true');
    }
}

// --- Provenance Modal (called via onclick) ---
...
} catch (e) { console.warn('Header scroll effect failed:', e); }
...
} catch (e) { console.warn('Banner close failed:', e); }
...
try {
    ...
    function isMobileMenuBusy() {
        const busy = Date.now() < mobileMenuBusyUntil;
        return busy;
    }
    ...
    function setMenuLevel(level) {
        ...
        if (slider) slider.style.setProperty('--menu-x', `${safeLevel * -100}vw`);
    }
    ...
    // Simple click handler for hamburger button
    if (btn) {
        ...
        console.error('[S&S Debug] Hamburger button NOT FOUND!');
    }
...
} catch (e) { console.warn('Mobile menu failed:', e); }
...
} catch (e) { console.warn('Modal escape handler failed:', e); }
...
} catch (e) { console.warn('Navigation highlighting failed:', e); }
...
} catch (e) { console.warn('Form validation failed:', e); }
...
} catch (e) { console.warn('Reveal animation failed:', e); }
...
} catch (e) { console.warn('Parallax effect failed:', e); }

// --- Mega Menu (Desktop Only) ---
try {
    (function megaMenuIntent() {
        // Desktop-only: keep tablets on their own nav for now
        ...
        // Click outside closes (passive to avoid blocking other interactions)
        document.addEventListener('pointerdown', (e) => {
            if (!root.contains(e.target)) close(0);
        }, { passive: true });
    })();
} catch (e) { console.warn('Mega menu failed:', e); }

// Final initialization complete

// (Blog-specific behaviors are handled by the shared modules above.)
