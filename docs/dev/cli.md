...
Use the `--theme` flag to specify which theme to use during initialization:

```bash
sum init acme-kitchens --theme theme_b
```

The default theme is `theme_a` if no `--theme` flag is provided.
