...
def test_process_steps_block_structure():
    ...
    assert block.child_blocks['steps'].child_block.child_blocks.keys() == {'number', 'title', 'description'}
...
def test_faq_block_structure():
    block = FAQBlock()
    assert block.child_blocks.keys() == {'eyebrow', 'heading', 'intro', 'items', 'allow_multiple_open'}
    assert block.child_blocks['items'].child_block.child_blocks.keys() == {'question', 'answer'}
