...
def test_process_steps_renders_theme_template(
    ...
    steps_list = soup.find("ol")
    assert steps_list is not None
    items = steps_list.find_all("li")
    assert len(items) == 5
    assert all("border-l" in _get_classes(item) for item in items)

    steps_text = steps_list.get_text()
    assert "01" in steps_text
    assert "07" in steps_text
